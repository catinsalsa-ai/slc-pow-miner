...
export const config = {
  ...
  cudaPersistent: boolEnv('CUDA_PERSISTENT', true),
  cudaThreads: Math.max(0, Math.floor(numEnv('CUDA_THREADS', 0))),
  cudaBlocks: Math.max(0, Math.floor(numEnv('CUDA_BLOCKS', 0))),
  cudaBlocksMult: Math.max(0, Math.floor(numEnv('CUDA_BLOCKS_MULT', 0))),
  stateCacheMs: Math.max(0, Math.floor(numEnv('STATE_CACHE_MS', 2500))),
  logEverySec: Math.max(1, Math.floor(numEnv('LOG_EVERY_SEC', 5))),
